...
yepnope(["${ctx}/resources/extjs/ux/treegrid/treegrid.css",
...
/** 顶部工具栏 */
role.tbar = [role.addAction, '-', role.editAction, '-', role.deleteAction, '-', role.searchField];
/** 底部工具条 */
	...
	if (treeNode.hasChildNodes()) {
		for (i = 0; i < treeNode.childNodes.length; i++) {
			if (treeNode.childNodes[i].getUI().checkbox) {
				if (treeNode.childNodes[i].getUI().checkbox.checked) {
					// 去除前缀
					role.childNodes += treeNode.childNodes[i].id.replace("_authority_", "") + ',';
				}
			}
		}
		for (i = 0; i < treeNode.childNodes.length; i++) {

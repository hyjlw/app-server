...
public class SiteController {
	...
	public Object all(ExtPager pager, @RequestParam(required = false) String name) {
		try {
			...
			List<Site> list = this.siteService.selectByExample(criteria);
			int total = this.siteService.countByExample(criteria);
	...
	public Object save(Site site) {
		try {
			...
			Criteria criteria = new Criteria();
			criteria.put("site", site);
			String result = this.siteService.saveSite(criteria);
			if ("01".equals(result)) {
	...
	public Object delete(@PathVariable String id) {
		try {
			...
			Criteria criteria = new Criteria();
			criteria.put("id", id);
			String result = this.siteService.delete(criteria);
			if ("01".equals(result)) {
